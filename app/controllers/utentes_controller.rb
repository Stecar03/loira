class UtentesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @utente.update(utente_params)
        format.html { redirect_to @utente, notice: "L'utente è stato modificato." }
  ...
  def destroy
    @utente.destroy
    respond_to do |format|
      format.html { redirect_to utentes_url, notice: "L'utente è stato eliminato." }
  ...
  private
    ...
    def utente_params
      params.require(:utente).permit(:cognome, :nome, :indirizzo, :numero, :annotazioni)
    end
end
